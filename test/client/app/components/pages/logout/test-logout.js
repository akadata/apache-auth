...
import Logout from '../../../../../../src/client/app/components/pages/logout';

test('Unsuccessful call to Apache logout endpoint on mount', (t) => {
  const clock = sinon.useFakeTimers();
  const requestStub = sinon.stub(request, 'post', (opts, cb) => {
    t.equal(opts.url, '/api/logout/logout', 'Logout API endpoint is correct');
  ...
  t.deepEqual(logout.state(), {status: null}, 'State is initially empty');
